*** Settings ***
Documentation     This is the main keywords for the Asenso app
Resource          pages/login_page.robot
Resource          pages/home_page.robot
Resource          pages/signup_page.robot

*** Keywords ***
Navigate to the login page
    [Arguments]                          ${url}    
    ...                                  ${locator}
    login_page.Navigate to login page    ${url}
    login_page.Verify page loaded        ${locator}

Click the signup button
    [Arguments]                          ${locator}
    signup_page.Verify page loaded        ${locator}
    signup_page.Click button              ${locator}

Validate text displayed
    [Arguments]                           ${expected_text}
    signup_page.Validate text displayed    ${expected_text}

Click the Citizenship dropdown
    [Arguments]                          ${locator}
    signup_page.Verify page loaded        ${locator}
    signup_page.Click button             ${locator}

Select Citizenship
    [Arguments]                          ${locator}
    signup_page.Verify page loaded       ${locator}
    signup_page.Click button             ${locator}

Text should be displayed
    [Arguments]                          ${locator}
    ...                                  ${text}
    signup_page.Verify page loaded       ${locator}
    signup_page.Validate value displayed  ${locator}
    ...                                  ${text}

Enter
    [Arguments]                          ${locator}
    ...                                  ${text}
    signup_page.Verify page loaded       ${locator}
    signup_page.Enter credential         ${locator}    
    ...                                  ${text}

Click the consent checkbox
    [Arguments]                          ${locator}
    signup_page.Verify page loaded       ${locator}
    signup_page.Click button             ${locator}

Click the Continue button
    [Arguments]                          ${locator}
    signup_page.Verify page loaded       ${locator}
    signup_page.Click button             ${locator}